  ...
  (if-let
    [plat-dir
     (if-let [sdk-root (or (System/getenv "ANDROID_SDK_ROOT")
       ...
       (first (filter #(.exists (io/file % "android.jar"))
                      (reverse (sort (.listFiles (io/file sdk-root "platforms"))))))
       nil)]
    (str (.getPath plat-dir) "/android.jar")
    (main/abort "Could not find android.jar file, make sure that an Android SDK"
